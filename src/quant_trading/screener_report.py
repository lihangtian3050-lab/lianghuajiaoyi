...
STYLE = """
  <style>
    :root {
      ...
      --blue:#2563eb; --blue-soft:#edf4ff; --red:#b42318; --green:#16803c;
      --amber:#b45309; --amber-soft:#fff7ed; --graphite:#111827;
      --shadow:0 10px 28px rgba(15,23,42,.07);
    }
    * { box-sizing:border-box; }
    body {
      margin:0; color:var(--ink); font-family:"Microsoft YaHei","Segoe UI",Arial,sans-serif;
      background:linear-gradient(180deg,#f8fbff 0,#f4f7fb 44%,#eef3f8 100%);
    }
    header { background:#fff; border-bottom:1px solid var(--line); padding:18px 28px; }
    main { width:min(1680px, calc(100vw - 48px)); margin:0 auto; padding:18px 0 28px; }
    ...
    .muted, .meta { color:var(--muted); line-height:1.65; }
    ...
    .topnav a, .button {
      ...
      display:inline-flex; align-items:center; min-height:36px; color:var(--blue); text-decoration:none;
    }
    .pill {
    ...
    .workbench { display:grid; grid-template-columns:300px minmax(0,1fr); gap:16px; align-items:start; }
    ...
    .metric strong { display:block; font-size:20px; line-height:1.25; overflow-wrap:anywhere; font-variant-numeric:tabular-nums; }
    ...
    .card::before { content:""; position:absolute; inset:0 auto 0 0; width:4px; background:linear-gradient(180deg,var(--blue),var(--green)); }
    ...
    .score { font-size:21px; font-weight:700; color:var(--blue); text-align:right; font-variant-numeric:tabular-nums; }
    ...
    @media (max-width: 860px) {
      ...
      .cards, .facts, .metric-list, .steps, .stock-summary { grid-template-columns:1fr; }
    }
  </style>
"""
